const baseUrl = process.env.SMOKE_BASE_URL || 'http://127.0.0.1:3300';
...
const adminCookie = await login('admin@sales-crm.local', 'admin123');
const managerCookie = await login('manager@sales-crm.local', 'manager123');
const analystCookie = await login('analyst@sales-crm.local', 'analyst123');

await expectJson('/api/dashboard', adminCookie);
await expectJson('/api/entities/accounts', analystCookie);
await expectJson('/api/documents', analystCookie);
await expectStatus('/api/documents/upload', analystCookie, 405);

const accounts = await expectJson('/api/entities/accounts', managerCookie);
const rowId = accounts.rows[0].id;

const approveResponse = await fetch(`${baseUrl}/api/entities/accounts/approve`, {
  method: 'POST',
  headers: {
...
const forbiddenApprove = await fetch(`${baseUrl}/api/entities/accounts/approve`, {
  method: 'POST',
  headers: {
